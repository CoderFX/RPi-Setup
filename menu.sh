...
do
  ...
  echo -------------------------------
  echo -e '0: Mount USB drive to read settings'
  echo -e '1: Change default password for pi'
  echo -e '2: Run raspi-config and setup hostname, overclocking, other settings'
  echo -e '3A: Create RSA 4096 bit SSH keys for RPi and copy them to USB for safe keeping (I suggest you save it in the default location (/home/pi/.ssh/id_rsa) by just hitting Enter)'
  echo -e '3B: Copy RSA keys from USB and set them working.'
  ...
  #0: Mount USB drive to read settings
  elif nocase "$opt" "0"
  then
  ...
  #3A: Create RSA 4096 bit SSH keys for RPi and copy them to USB for safe keeping
  #Guide: https://www.raspberrypi.org/documentation/remote-access/ssh/passwordless.md
  elif nocase "$opt" "3A"
  then
    sudo ./setup-ssh-keys.sh

  #3B: Copy RSA keys from USB and set them working
